...
requests = [ \
    dsc = UDS()/UDS_DSC(diagnosticSessionType=0x2), \
    sa = UDS()/UDS_SA(securityAccessType=2, securityKey=b'\x11'), \
    ru = UDS()/UDS_RU(memorySizeLen=0x1, memoryAddressLen=0x4, memoryAddress4=0xaaaaaaaa, memorySize1=0x64), \
    td = UDS()/UDS_TD(blockSequenceCounter=0x0, transferRequestParameterRecord=data), \
    rte = UDS()/UDS_RTE(transferRequestParameterRecord=b'done')
]
...
for req in requests:
    ...
    if hasattr(reqp, 'negativeResponseCode'):
        print("Update failed :(")
        exit(-1)
    else:
        print(req.service + " ... OK")
